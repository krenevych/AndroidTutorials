...
Перш ніж перелічувати константи, давайте визначимось, що взагалі означають ці поняття:

* **Action (Дія)** — відповідає на питання **"Що зробити?"**. Це рядок, який описує загальний намір (наприклад: "переглянути", "відправити", "зателефонувати"). Це найголовніша характеристика інтенту.
* **Category (Категорія)** — відповідає на питання **"В яких умовах / в якому контексті?"**. Це додаткова інформація про те, яким саме повинен бути компонент, що обробить дію (наприклад: "має бути в лаунчері", "безпечно відкривати з веб-браузера" або "це стандартний обробник").
* **Data (Дані)** — відповідає на питання **"З чим ми працюємо?"**. Вказує на тип інформації (текст, фотографія, PDF-файл, конкретне веб-посилання).

> **Чи потрібні Intent Filters для явних інтентів?**
> Ні. Якщо ваша Activity призначена для запуску **тільки зсередини вашого ж застосунку** (через явний інтент `Intent(this, SecondActivity::class.java)`), вам **взагалі не потрібно писати блок `<intent-filter>`** у файлі `AndroidManifest.xml`. Явні інтенти ігнорують будь-які фільтри та категорії. Достатньо простого оголошення `<activity android:name=".SecondActivity" />`.

Щоб правильно налаштувати Intent Filter для **неявних** інтентів, потрібно знати стандартні константи, які використовує Android:

**Популярні `action` (Дії):**
* `android.intent.action.MAIN` — вказує, що це головна точка входу в програму. **Це означає, що цей екран запускається найпершим**, коли користувач натискає на іконку застосунку на робочому столі телефону. Ця дія не очікує жодних вхідних даних. *(Примітка: У вашому `AndroidManifest.xml` може бути декілька Activity з дією `MAIN` та категорією `LAUNCHER`. У такому разі на робочому столі телефону з'явиться декілька іконок для вашого застосунку, кожна з яких відкриватиме відповідний екран).*
...
* `android.intent.category.DEFAULT` — **обов'язкова категорія для всіх неявних інтентів** (окрім `MAIN`). Коли ви викликаєте `startActivity(intent)`, Android автоматично додає до вашого інтенту категорію `DEFAULT`. Тому, якщо ваша Activity (наприклад, `ShareActivity`) хоче, щоб її знайшли і запустили через неявний інтент, вона **повинна** мати цю категорію у своєму фільтрі, інакше система пройде повз неї.
