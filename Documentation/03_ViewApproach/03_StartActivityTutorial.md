...
* `android.intent.category.LAUNCHER` — вказує, що іконка цієї Activity має відображатися у списку всіх програм на робочому столі смартфона (Launcher). Зазвичай використовується в парі з `ACTION_MAIN`.
* `android.intent.category.DEFAULT` — **обов'язкова** категорія для будь-якої Activity, яка хоче приймати неявні інтенти (окрім `MAIN`). Якщо ви забудете додати `DEFAULT` у свій фільтр (наприклад, для обробки посилань), система вас просто проігнорує.
