...
та натиснути Enter — це відкриє інтерактивну сесію, де ви зможете вводити команди так, ніби працюєте безпосередньо у терміналі на самому телефоні.

### Пошук та фільтрація тексту (`grep`) та символ конвеєра (`|`)

Утиліта **`grep`** є ключовим інструментом для пошуку потрібних даних (особливо при роботі з логами або списками процесів). Вона шукає вказаний рядок у великих об'ємах тексту. 
Найчастіше вона використовується разом із символом конвеєра **`|`** (pipe), який бере результат виконання першої команди і "передає" його на вхід команді `grep` для фільтрації.

* **Базовий синтаксис:** `команда | grep "слово"`
* **Ігнорувати регістр (`-i`):** `команда | grep -i "СлОвО"`
* **Знайти кілька слів одночасно (`-E`):** `команда | grep -E "слово1|слово2"`
* **Інвертований пошук (НЕ містить) (`-v`):** `команда | grep -v "виключити_це"`

_Приклади застосування:_
```bash
# Знайти всі процеси, що містять 'example' (adb shell)
adb shell "ps -A | grep example"

# Фільтрувати системні логи, залишаючи лише ті, що містять 'NetworkError' (локальний комп'ютер)
adb logcat | grep "NetworkError"
```
...
* <a href="#section4">4. Аналіз процесів та ресурсів (Linux рівень)</a>
* <a href="#section5">5. Системні логи (logcat)</a>
* <a href="#section6">6. Обмін файлами та файлова система</a>
* <a href="#section7">7. Симуляція дій користувача (input)</a>
* <a href="#section8">8. Корисні налаштування розробника</a>
...
<a name="section4"></a>
## 4. Аналіз процесів та ресурсів (Linux рівень)
...
* Відобразити список усіх активних процесів у системі:
  ...
  adb shell ps
  ```
  Корисні параметри утиліти `ps`:
  * `-A` (або `-e`) — показує **абсолютно всі процеси**, а не лише ті, що запущені від імені користувача оболонки (`shell`). Необхідно, щоб побачити процеси Android додатків.
  * `-o PID,NAME` — форматує вивід, залишаючи лише вказані колонки (ідентифікатор процесу та його назву). Це зручно для подальшої фільтрації списку.
  
  Приклад (сформувати чистий список із двох колонок):
  ```bash
  adb shell ps -A -o PID,NAME
  ```
  Приклад (сформувати чистий список із двох колонок і відфільтрувати за словом `"example"`):
  ```bash
...
<a name="section6"></a>
## 6. Обмін файлами та файлова система
...
<a name="section7"></a>
## 7. Симуляція дій користувача (`input`)
...
<a name="section8"></a>
## 8. Корисні налаштування розробника
...
<a name="section10"></a>
## 10. Практичні сценарії налагодження та ТОП команди

### ТОП-10 команд для щоденної роботи
